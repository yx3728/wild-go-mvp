...
- **July 17 fidelity passes** reduce the Capture resting card angle from `3.8`/`6` degrees to `1.1`/`2` degrees (planar/Y-axis), lower its complete control/CTA block, rebalance Binder's board/guide proportions, and enlarge the Profile showcase fan. Current concept scores: Capture composite `0.830` (thumb `0.866`, hist `0.644`, bands `0.900`); Binder `0.826` / `0.859` / `0.599` / `0.934`; Friends `0.838` / `0.832` / `0.741` / `0.928`. Small color/band tradeoffs are accepted under the explicit UI/layout-first priority because the spatial layout measures improve.
  ...
  - **Binder:** `BinderLeatherSurface` pebble-grain leather + top lime glow; C-shaped silver rings through circular grommets (two rings); serif "Wild Go" lockup; wider progress track and bolder "134 Cards"; thicker Lv.23 hex stroke; softer rarity-guide inset on leather.

...
- The July 13 Friends/Profile geometry pass fixed the three viewport defects found against `friends-showcase-stack.png`: the second Friend Activity row's locality/date line no longer hides behind the action rail, the hero showcase card no longer overlaps the "Drag to showcase" pill, and the white Profile bottom bar now uses the concept's "Collection" label. Friends concept fidelity rose from `0.778` to `0.801` (tracked as `swiftui-native-friends-profile-v17.png` with raised gates). Capture and Binder tracked references were refreshed from the current ShaderKit build (capture composite `0.780`, binder `0.832`) with gates raised to match.
- Resume from here by configuring a live Supabase project, verifying AVFoundation on physical hardware, training the local Core ML model with a labeled dataset, and re-running `npm run ios:interactions` in a GUI session (the coordinate suite needs a visible Simulator window plus Accessibility permission; the static `ios:verify-events` gate and all Simulator-free audits pass). Source-image similarity is intentionally not the current optimization target.

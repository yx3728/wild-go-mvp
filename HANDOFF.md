...
- **July 17 Capture fidelity pass** reduces the resting card angle from `3.8`/`6` degrees to `1.1`/`2` degrees (planar/Y-axis) and moves the CTA stack down `18` pt on compact-height phones. Current concept scores: Capture composite `0.825` (thumb `0.859`, hist `0.640`, bands `0.897`); Binder `0.818` / `0.844` / `0.601` / `0.937`; Friends `0.824` / `0.820` / `0.727` / `0.911`. The small histogram tradeoff is accepted under the explicit UI/layout-first priority; the other three Capture measures improve.

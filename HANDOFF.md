...
- The July 13 Friends/Profile geometry pass fixed the three viewport defects found against `friends-showcase-stack.png`: the second Friend Activity row's locality/date line no longer hides behind the action rail, the hero showcase card no longer overlaps the "Drag to showcase" pill, and the white Profile bottom bar now uses the concept's "Collection" label. The layout-weighted concept scores are Capture `0.816`, Binder `0.855`, and Friends `0.814`; Binder vertical-band similarity improved from `0.954` to `0.956` and Capture's spatial thumbnail score improved to `0.837`.
...
- After the Capture resting-angle change, the Simulator build, fresh screenshot, native visual gate, static 37-event map, aggregate verification, and concept audit all pass. A fresh full coordinate rerun still needs an unlocked macOS session: the app and device booted, but both AppleScript and Computer Use confirmed that the locked desktop exposed no Simulator window to click. The last complete five-suite coordinate pass is commit `50df19f`.
...
- Resume from here by configuring a live Supabase project, verifying AVFoundation on physical hardware, and training the local Core ML model with a labeled dataset. Source-image similarity is intentionally not the current optimization target.
...
- Friends/Profile now separates that five-action rail from a four-item white bottom navigation like the concept. The white bar labels the binder destination "Collection" to match the reference. Its tracked `v17` score is composite `0.801`, thumbnail `0.786`, histogram `0.748`, and vertical bands `0.930`, with raised regression gates.
...
- Friends/Profile's showcase deck uses a `228 x 338` pt hero card, a `208` pt image window, and taller supporting cards. Its former Sticker overlay is superseded by the shared six-star ShaderKit rainbow-holo material.

...
- **July 17 fidelity passes** reduce the Capture resting card angle from `3.8`/`6` degrees to `1.1`/`2` degrees (planar/Y-axis), move its CTA stack down `18` pt on compact-height phones, and rebalance Binder's board/guide proportions. Current concept scores: Capture composite `0.825` (thumb `0.859`, hist `0.640`, bands `0.897`); Binder `0.826` / `0.859` / `0.599` / `0.934`; Friends `0.824` / `0.820` / `0.727` / `0.911`. Small color/band tradeoffs are accepted under the explicit UI/layout-first priority because the spatial layout measures improve.
  ...
  - **Friends/Profile:** green serif "Wild Go" without leaf ornament; thinner avatar ring and XP bar; thinner Cards/Places icons; deck scaled to ~`0.94` with `210×344` hero / `244` pt photo / `108×252` supporting cards inside a `350` pt frame; smaller 6★ / URBAN LEGEND badges.
...
- The Profile showcase deck (July 14) uses a `350` pt frame at `0.94` scale, `210×344` pt hero, `244` pt photo window, and `108×252` pt supporting cards. Fan offsets, rotations, dropped-state transforms, Drag/Flip identifiers, and activity/rail geometry from earlier July 13 passes remain intact. This supersedes the prior `358` / `228×380` / `250` / `118×274` deck proportions.
...
- Friends/Profile's showcase deck (July 14) uses a `350` pt frame at `0.94` scale, a `210×344` pt hero, a `244` pt image window, and `108×252` pt supporting cards. These supersede the earlier `358` / `228×380` / `250` / `118×274` and `228×338` / `208` geometries. Its former Sticker overlay is superseded by the shared six-star ShaderKit rainbow-holo material.

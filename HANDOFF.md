...
- [`Sticker`](https://github.com/bpisano/Sticker) is wired into the native SwiftUI target as an SPM dependency for Pokemon-style Metal foil shaders and motion-driven shimmer. Native foil art now uses Sticker-driven border, card-surface, and constrained spectral photo layers with the package README's example shader parameters instead of hand-written gradient art. `Package.resolved` pins Sticker `1.4.0`, and `npm run goal:audit` now fails if the project loses the dependency, README example parameters, motion path, shader precompile call, or sticker QA reference. On iOS 18+, Sticker shaders are precompiled on launch.
...
- Capture's metadata surface now shows the Sticker layer at `0.16` presentation opacity instead of `0.40`, preserving the README example parameters while revealing the concept's darker card base. The accepted pass reaches composite `0.773`, thumbnail `0.832`, histogram `0.628`, and bands `0.894` after measured `0.28` and `0.16` trials.
...
- The Profile hero's Sticker frame is now shown at `0.95` opacity without changing the package's README example parameters. This lifts composite fidelity to `0.778`, histogram to `0.706`, and bands to `0.910`; the matching Binder opacity experiment was reverted because it produced no composite gain.

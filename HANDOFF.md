...
## Latest Handoff — July 22, 2026

- Working branch: `codex/native-mvp-stack-sync`
- GitHub PR: <https://github.com/yx3728/wild-go-mvp/pull/1>
- **Shippable native MVP boundary:** Capture and Binder are live. Map and Friends/Profile route to clear, interactive Coming Soon screens; AI identification is represented as a future-state label instead of a simulated result.
- Capture is now an explicit two-stage flow: AVFoundation camera or PhotosUI import normalizes the JPEG into an on-device photo draft, then `Save to Binder` inserts its `WildObservation` into SwiftData. The app navigates to Binder only after that save succeeds.
- Binder prioritizes every locally captured card ahead of starter examples, reports the real on-device saved count, and renders the persisted photo instead of replacing it with demo artwork.
- Capture no longer requests location on launch. The CoreLocation service is lazy and remains available for later map/location work without interrupting this MVP with an unrelated permission prompt.
- Clean uninstall/reinstall verification passed on iPhone 17 Pro Simulator with no location prompt. Real-coordinate QA passed Map, Capture, Binder, and Friends placeholder controls, including four-card swipes, tilt/depth/flip, photo preparation, explicit save, automatic Binder navigation, persisted-photo detection, and sharing.
- Workspace npm dependencies were updated within their declared ranges. `npm outdated` is clean after the lockfile refresh.
- Native stack is SwiftUI + SwiftData + AVFoundation + MapKit + PhotosUI + CoreLocation. Supabase/Postgres/Storage/Edge Functions remain the cloud backend, with cloud species recognition first and Vision/Core ML fallback for the bundled starter model.
- Supabase CLI is a project-local, exact `2.109.1` development dependency. `npm run supabase:verify` runs 18 Deno handler/contract tests, type-checks the deployed entrypoint, verifies the CLI, validates shell syntax, and enforces the hosted-secret deployment contract. The deploy helper publishes only the OpenAI key and model; hosted Supabase supplies reserved `SUPABASE_*` runtime values.
- Signed-in collection sync treats Storage upload and Postgres metadata as one ordered operation. If the local JPEG cannot upload, the card remains pending instead of being counted as synced.
- Card materials use ShaderKit `1.2.4` across all six rarity tiers: matte steel, colored alloy, crosshatched silver, iridescent pearl, inverted foil, and rainbow holo.
- **July 17 fidelity passes** reduce the Capture resting card angle from `3.8`/`6` degrees to `1.1`/`2` degrees (planar/Y-axis), lower its complete control/CTA block, rebalance Binder's board/guide proportions, and enlarge the Profile showcase fan. Current concept scores: Capture composite `0.830` (thumb `0.866`, hist `0.644`, bands `0.900`); Binder `0.826` / `0.860` / `0.597` / `0.934`; Friends `0.838` / `0.832` / `0.741` / `0.929`. Small color/band tradeoffs are accepted under the explicit UI/layout-first priority because the spatial layout measures improve.
  - **Capture:** resting card angle `1.1°` clockwise with `2°` Y-axis depth; compact-height interaction block moved down `12` pt with CTA spacing retained at `18` pt; LeafRule thinner/longer; darker desaturated unlock background; latin name olive-green; primary CTA flat olive `RGB(92,112,33)` with no lime border; Share Card translucent black fill + white border; rainbow-holo shader foil/glitter/crosshatch intensities raised for a denser foil read.
  - **Binder:** `BinderLeatherSurface` pebble-grain leather + top lime glow; three C-shaped silver rings through circular grommets at concept-measured vertical anchors; serif "Wild Go" lockup; wider progress track and bolder "134 Cards"; thicker Lv.23 hex stroke; softer rarity-guide inset on leather.
  - **Friends/Profile:** green serif "Wild Go" without leaf ornament; thinner avatar ring and XP bar; thinner Cards/Places icons; deck scaled to `1.04` with `210×344` hero / `244` pt photo / `108×252` supporting cards inside a `350` pt frame; smaller 6★ / URBAN LEGEND badges.
  - Remaining non-code gaps for pixel-identity: Simulator Dynamic Island chrome, concept vs device aspect ratio, and concept static cracked-foil texture vs live ShaderKit materials. Next visual work should target one screen/element at a time rather than wholesale score chasing.
- Capture retains drag-responsive shader tilt, press-and-hold depth, independent front/back flip, and four-card swipe pagination.
- Capture's compact-height interaction strip now uses `6` pt top padding instead of `-6`, moving the strip, four page dots, and both CTAs down together to the source position. All four concept measures improve from `0.825` / `0.859` / `0.640` / `0.897` to `0.830` / `0.866` / `0.644` / `0.900`; repeated renders remain stable and the complete Capture coordinate suite passes.
- Binder's balanced grid now gives the collection more of the concept's vertical weight: the board is `492` pt tall, feature cards are `289`/`281` pt, and the four supporting cards are `175` pt. Photo windows scale from those card heights. A tighter `4` pt content rhythm and `66` pt six-level rarity guide keep the complete guide and single-line Tilt/Binder Tips row above the bottom navigation without dropping card names, rarity, confidence, locality, or dates. Composite improves `0.818` to `0.826` and thumbnail layout `0.844` to `0.859`; the full Binder coordinate suite passes.
- Binder now matches the reference's three visible metal rings instead of rendering two decorative rings. The rings use proportional vertical anchors (`10.5%`, `32.5%`, `83.3%`) inside explicit `492`/`482` pt grid/list board heights, so they align with the feature and supporting card rows without influencing SwiftUI layout proposals. The stable render keeps the tracked `0.826` / `0.859` / `0.599` / `0.934` concept scores, and the complete Binder coordinate suite passes after the change.
- Binder's card grid now uses `6` pt more horizontal space and sits `3` pt farther left, moving the combined card bounds from roughly `90%` toward the reference's `93%` screen coverage without changing card heights, ring anchors, or hit targets. Repeat renders raise spatial thumbnail fidelity from `0.858822` to `0.860298` and composite from `0.825630` to about `0.82606`; histogram moves to `0.5968`-`0.5971`, accepted under the explicit UI/layout-first priority. The complete Binder coordinate suite remains green.
...
- The Profile showcase deck now uses a `350` pt frame at `1.04` scale, `210×344` pt hero, `244` pt photo window, and `108×252` pt supporting cards. The scale-only pass improves all four concept measures from `0.824` / `0.820` / `0.727` / `0.911` to `0.838` / `0.832` / `0.741` / `0.928`. Fan offsets, rotations, dropped-state transforms, Drag/Flip identifiers, and activity/rail geometry remain intact; two renders are pixel-identical and the full Profile coordinate suite passes.
- Profile's lower navigation now matches the source's full-width bottom sheet instead of floating as a four-corner capsule. The Profile-only surface removes its `12` pt outer margins and `6` pt bottom gap, keeps `30` pt top corners, and leaves all four tab item frames unchanged. Two renders are pixel-identical; composite improves from `0.837885` to `0.838043`, thumbnail from `0.831838` to `0.831976`, and bands from `0.928479` to `0.928913`, while histogram remains effectively level at `0.741141`.
...
Wild Go is a mobile-first prototype for a real-world urban nature collection app. In the current MVP, users photograph nearby plants and animals and collect each discovery as an original card built from their own photo. AI-assisted identification remains visible as a deliberate future-state placeholder.
...
- SwiftData local persistence, AVFoundation camera preview and still capture, and PhotosUI import. MapKit, CoreLocation, and the existing recognition architecture remain available for post-MVP work while their user-facing destinations are placeholders.
...
- Camera capture is Simulator-hardened: `CameraSession` skips configuration on Simulator, waits briefly for a ready photo connection on device, and prevents overlapping captures. Simulator photo generation and device capture both produce a draft first; only the explicit `Save to Binder` action persists the observation.
...
- Friends/Profile's showcase deck uses a `350` pt frame at `1.04` scale, a `210×344` pt hero, a `244` pt image window, and `108×252` pt supporting cards. The July 17 scale correction supersedes the prior `0.94` presentation while preserving the intrinsic geometry, fan offsets, and shared six-star ShaderKit rainbow-holo material.
...
1. Optional: continue concept polish one screen or element at a time (Capture foil texture, Binder ring placement, Friends deck scale). Prefer side-by-side visual judgment over raising audit gates alone.
2. Configure live Supabase project values and secrets for end-to-end cloud recognition/storage. **Tooling provided:**

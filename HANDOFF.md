...
- **July 14 concept-fidelity pass** aligned Capture, Binder, and Friends/Profile to the three concept images more closely. Tracked references and audit gates were refreshed from the latest Simulator smoke shots. Current concept scores: Capture composite `0.818` (thumb `0.847`, hist `0.643`, bands `0.895`); Binder `0.818` / `0.844` / `0.601` / `0.937`; Friends `0.824` / `0.820` / `0.727` / `0.911`. `npm run verify` and `npm run ios:interactions` are green.
  - **Capture:** resting card tilt `3.8°` clockwise; LeafRule thinner/longer; darker desaturated unlock background; latin name olive-green; primary CTA flat olive `RGB(92,112,33)` with no lime border; Share Card translucent black fill + white border; rainbow-holo shader foil/glitter/crosshatch intensities raised for a denser foil read.
